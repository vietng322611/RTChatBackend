    ...
    RedisOptions options) : IPresenceService
{
        ...
        string connectionId)
    {
        ...
        if (_presenceTtl != TimeSpan.Zero) await _redis.KeyExpireAsync(key, _presenceTtl);
    ...
    public async Task<bool> SetOfflineAsync(Guid userId, string connectionId)
    {
        ...
        // Refresh TTL if still online
        if (_presenceTtl != TimeSpan.Zero && connectionCount > 0) await _redis.KeyExpireAsync(key, _presenceTtl);
